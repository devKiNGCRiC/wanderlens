-- Wanderlens Chat — Phase 5 (Share a location into chat)
-- Adds a `location` message type carrying a coordinate + reverse-geocoded
-- label, rendered as a card (no live map thumbnail in the bubble — that
-- would mean mounting a native MapLibre view per message in a scrolling
-- list, which is expensive; tapping opens the device's own maps app instead).
...
alter table public.messages add constraint messages_message_type_check
  check (message_type in ('text', 'image', 'gallery', 'spot', 'location'));
...
-- ============================================================
-- RPCs: carry location fields alongside everything already there
-- ============================================================
...
-- list_conversations: a shared-location last message shows the label
