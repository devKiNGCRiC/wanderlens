-- Wanderlens Chat — Document messages
-- Reuses the existing private message-media bucket (already RLS-scoped to
-- conversation membership) — no new bucket needed. An audio file picked
-- through the document picker is NOT sent as a 'document' — the client
-- routes it through the existing 'voice' pipeline instead so it gets the
-- same cassette-player treatment as a recorded voice note; 'document' here
-- covers everything else (PDF, docx, etc.) and gets a generic file card.
...
alter table public.messages add constraint messages_message_type_check
  check (message_type in ('text', 'image', 'gallery', 'spot', 'location', 'video', 'voice', 'document'));
...
alter table public.messages add constraint messages_content_or_media_check
  check (
    ...
    or (message_type = 'document' and media_path is not null and file_name is not null)
  );

-- ============================================================
-- RPCs: carry file_name/file_size alongside everything already there
-- ============================================================
...
begin
  ...
  return query
    select
      m.id, m.sender_id, m.content, m.message_type, m.media_path, m.video_duration_seconds, m.voice_duration_seconds, m.file_name, m.file_size, m.gallery_layout, m.client_generated_id, m.created_at,
...
-- list_conversations: a document last message shows the file name
...
as $$
  select
    ...
    coalesce(
      ...
      case
        ...
        when lm.message_type = 'document' then '📄 ' || coalesce(lm.doc_name, 'Document')
        else null
      end
    ),
    lm.created_at,
  ...
  left join lateral (
    select m.content, m.message_type, m.created_at, m.sender_id, m.file_name as doc_name,
