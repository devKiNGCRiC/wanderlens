-- Wanderlens Chat — Voice note messages
-- Reuses the existing private message-media bucket (already RLS-scoped to
-- conversation membership) — no new bucket needed, .m4a recordings just land
-- there alongside photos and videos.

alter table public.messages add column if not exists voice_duration_seconds integer;

alter table public.messages drop constraint if exists messages_message_type_check;
alter table public.messages add constraint messages_message_type_check
  check (message_type in ('text', 'image', 'gallery', 'spot', 'location', 'video', 'voice'));
...
-- ============================================================
-- RPCs: carry voice_duration_seconds alongside everything already there
-- ============================================================
...
begin
  ...
  return query
    select
      m.id, m.sender_id, m.content, m.message_type, m.media_path, m.video_duration_seconds, m.voice_duration_seconds, m.gallery_layout, m.client_generated_id, m.created_at,
...
-- list_conversations: a voice note last message shows a mic preview line
