  ...
  async function handleSave() {
    ...
    try {
      const { data, error } = await supabase.from('trails').insert({
        ...
        genre_filter: genre === 'Any' ? null : genre,
        stops: trail.stops,
      }).select('id').single();
      if (error) throw error;
      setSavedId(data.id);
      Alert.alert('Trail saved', 'Find it anytime under My Trails.');
    } catch (err: any) {
    ...
    <ScreenBackground>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingTop: insets.top + 20, paddingBottom: 60 }}>
        <View style={styles.headerRow}>
          ...
          <Pressable onPress={() => router.push('/my-trails')} style={styles.myTrailsBtn}>
            <Ionicons name="bookmark-outline" size={14} color={theme.color.gold} />
          ...
          <View style={{ marginTop: 28 }}>
            ...
            <Pressable style={[styles.saveBtn, savedId && styles.saveBtnDone]} onPress={handleSave} disabled={saving || !!savedId}>
              {saving ? (
                ...
                <><Ionicons name="checkmark" size={15} color={theme.color.gold} /><Text style={styles.saveBtnText}>Saved to My Trails</Text></>
              ) : (
                <><Ionicons name="bookmark-outline" size={15} color={theme.color.gold} /><Text style={styles.saveBtnText}>Save this trail</Text></>
              )}
            </Pressable>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  myTrailsBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, borderWidth: 1, borderColor: theme.color.surface2, borderRadius: 18, paddingVertical: 7, paddingHorizontal: 12 },
