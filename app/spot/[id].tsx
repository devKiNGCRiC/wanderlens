...
export default function SpotDetail() {
  ...
  const load = useCallback(async () => {
    ...
    const { data: commentData } = await supabase
      .from('spot_comments')
      .select('id, content, user_id, profiles(username, full_name, avatar_url)')
      .eq('spot_id', id)
      ...
      <KeyboardAvoidingView style={{ flex: 1, }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 24 }} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.body}>
            ...
            {comments.map((c) => {
              const cHandle = c.profiles?.username || c.profiles?.full_name || 'traveler';
              return (
                <Pressable key={c.id} style={styles.commentRow} onPress={() => goToProfile(c.user_id)}>
                  <View style={styles.commentAvatar}>
                    {c.profiles?.avatar_url ? <Image source={{ uri: c.profiles.avatar_url }} style={styles.commentAvatarImage} /> : <Text style={styles.commentAvatarText}>{cHandle.charAt(0).toUpperCase()}</Text>}
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.commentName}>{cHandle}</Text>
...
const styles = StyleSheet.create({
  ...
  commentInputRow: { flexDirection: 'row', gap: 10, paddingTop: 12, marginBottom: 8, paddingHorizontal: 14, borderTopWidth: 1, borderTopColor: theme.color.surface2, backgroundColor: theme.color.dusk, alignItems: 'center' },
